...
public final class ClassInfo implements Comparable<ClassInfo> {
	...
	private void splitNames(RootNode root, boolean canBeInner) {
		...
		// $ character is not always a separator generated by the compiler;
		// it can appear in user-defined class name (e.g., $what$haha$$ is a valid class name)
		if (canBeInner && sep > 0 && sep != clsName.length() - 1) {
			...
			if (Character.isDigit(clsName.charAt(sep + 1))) {
				// for compiler generated anonymous inner class, which appear as com.example.Main$1, force a prefix $
				clsName = clsName.substring(sep);
			} else {
				// for named inner class, e.g., android.widgets.View.OnClickListener
