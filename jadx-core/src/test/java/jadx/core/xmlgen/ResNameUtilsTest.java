...
class ResNameUtilsTest {
	...
	@DisplayName("Check convertToJavaIdentifier(name)")
	@ParameterizedTest(name = "{0} -> {1}")
	@MethodSource("provideArgsForConvertToJavaIdentifierTest")
	void testConvertToJavaIdentifier(String name, String expectedResult) {
		assertThat(ResNameUtils.convertToJavaIdentifier(name)).isEqualTo(expectedResult);
	}
	...
	private static Stream<Arguments> provideArgsForConvertToJavaIdentifierTest() {
		return Stream.of(
